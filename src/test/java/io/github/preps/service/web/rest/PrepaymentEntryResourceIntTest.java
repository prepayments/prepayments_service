...
/**
 * Test class for the PrepaymentEntryResource REST controller.
 *
 * @see PrepaymentEntryResource
 */
@RunWith(SpringRunner.class)
@SpringBootTest(classes = {SecurityBeanOverrideConfiguration.class, PrepaymentsserviceApp.class})
public class PrepaymentEntryResourceIntTest {

    private static final String DEFAULT_ACCOUNT_NUMBER = "979671046494038";
    private static final String UPDATED_ACCOUNT_NUMBER = "88949108631";
    ...
    private static final String DEFAULT_SERVICE_OUTLET = "812";
    private static final String UPDATED_SERVICE_OUTLET = "915";
     ...
     *
     * @see io.github.preps.service.repository.search.PrepaymentEntrySearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public static PrepaymentEntry createEntity(EntityManager em) {
        PrepaymentEntry prepaymentEntry = new PrepaymentEntry().accountNumber(DEFAULT_ACCOUNT_NUMBER)
    ...
    public void createPrepaymentEntry() throws Exception {
        ...
        restPrepaymentEntryMockMvc.perform(post("/api/prepayment-entries").contentType(TestUtil.APPLICATION_JSON_UTF8).content(TestUtil.convertObjectToJsonBytes(prepaymentEntryDTO)))
    ...
    public void getAllPrepaymentEntriesByAmortizationEntryIsEqualToSomething() throws Exception {
        // Initialize the database
        AmortizationEntry amortizationEntry = AmortizationEntryResourceIntTest.createEntity(em);
    ...
    public void getNonExistingPrepaymentEntry() throws Exception {
        // Get the prepaymentEntry
        restPrepaymentEntryMockMvc.perform(get("/api/prepayment-entries/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
    }
    ...
    public void updatePrepaymentEntry() throws Exception {
        ...
        updatedPrepaymentEntry.accountNumber(UPDATED_ACCOUNT_NUMBER)
        ...
        restPrepaymentEntryMockMvc.perform(put("/api/prepayment-entries").contentType(TestUtil.APPLICATION_JSON_UTF8).content(TestUtil.convertObjectToJsonBytes(prepaymentEntryDTO)))
                                  .andExpect(status().isOk());
    ...
    public void deletePrepaymentEntry() throws Exception {
        ...
        restPrepaymentEntryMockMvc.perform(delete("/api/prepayment-entries/{id}", prepaymentEntry.getId()).accept(TestUtil.APPLICATION_JSON_UTF8)).andExpect(status().isOk());
    ...
    public void searchPrepaymentEntry() throws Exception {
        // Initialize the database
        prepaymentEntryRepository.saveAndFlush(prepaymentEntry);
        when(mockPrepaymentEntrySearchRepository.search(queryStringQuery("id:" + prepaymentEntry.getId()), PageRequest.of(0, 20))).thenReturn(
            new PageImpl<>(Collections.singletonList(prepaymentEntry), PageRequest.of(0, 1), 1));
